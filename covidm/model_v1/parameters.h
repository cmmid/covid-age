...
struct Observer
{
    ...
    bool operator()(PopulationParameters& params, unsigned int scenario, unsigned int run, double t, unsigned int dynamics);
    Rcpp::Function func;
    bool null;
};
...
bool Observer::operator()(PopulationParameters& pp, unsigned int scenario, unsigned int run, double t, unsigned int dynamics)
{
    if (null)
        return true;
    
    using namespace Rcpp;
    RObject observer_return = func(scenario, run, t, dynamics);
    ...
    if (is<List>(observer_return))
    {
        ...
        if (ret.containsElementNamed("csvlines"))
        {
            string csvlines = as<string>(ret["csvlines"]);
            cout << csvlines;
            if (csvlines.back() != '\n')
                cout << "\n";
        }

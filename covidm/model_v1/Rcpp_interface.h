...
// [[Rcpp::export]]
Rcpp::DataFrame cm_backend_simulate(Rcpp::List parameters, unsigned int n_run = 1, unsigned long int seed = 0)
{
    ...
    // Numeric data from all runs
    vector<vector<double>> data;
    unsigned int n_times = 0, n_populations = 0, n_age_groups = 0;
    double time0 = 0., dt = 1;
    ...
    for (unsigned int r = 0; r < n_run; ++r)
    {
        ...
        if (data.empty())
        {
            data.resize(rep.data.size(), vector<double>());
            n_times = rep.n_times;
            n_populations = rep.n_populations;
            n_age_groups = rep.n_age_groups;
            col_names = rep.col_names;
            time0 = P.time0;
            dt = P.time_step * P.report_every;
        }
        for (unsigned int col = 0; col < rep.data.size(); ++col)
            data[col].insert(data[col].end(), rep.data[col].begin(), rep.data[col].end());
    }

    // Create data frame with all results
    Rcpp::DataFrame results = Rcpp::DataFrame::create(
        Rcpp::Named("run") = Rcpp::rep_each(Rcpp::seq(1, n_run), n_times * n_populations * n_age_groups),
        Rcpp::Named("t") = Rcpp::rep(Rcpp::rep_each(time0 + Rcpp::seq(0, n_times - 1) * dt, n_populations * n_age_groups), n_run),
        Rcpp::Named("population") = Rcpp::rep(Rcpp::rep_each(Rcpp::seq(1, n_populations), n_age_groups), n_times * n_run),
        Rcpp::Named("group") = Rcpp::rep(Rcpp::seq(1, n_age_groups), n_times * n_populations * n_run)
    );

    for (unsigned int col = 0; col < data.size(); ++col)
        results.push_back(data[col], col_names[col]);

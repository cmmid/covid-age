...
cm_sample_fit = function(fit, n)
{
    ...
    for (i in 1:n)
    {
        ...
        # Run simulation
        dyn = cm_backend_simulate(parameters, 1, fit$options$model_seed);
        setDT(dyn);
        dyn = melt(dyn, id.vars = c("run", "t", "population", "group"), variable.name = "compartment", value.name = "value");
        dyn[, run := i];
...
{
    dyn = cm_backend_simulate(cm_translate_parameters(parameters), n, model_seed);
    setDT(dyn);
    dyn = melt(dyn, id.vars = c("run", "t", "population", "group"), variable.name = "compartment", value.name = "value");
    dyn = cm_annotate_dynamics(dyn, parameters);

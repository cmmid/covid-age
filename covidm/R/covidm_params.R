    ...
    seed_times = NULL, dist_seed_ages = NULL, observer = NULL, schedule = NULL)
{
    ...
    assign_g = function(pop, name, value, n_groups) {
        if (!is.null(value)) {
            if (length(value) == 1 | length(value) == n_groups) {
                ...
                stop(paste0(name, " must be of length 1 or n_groups."));
            }
        }
    }
    ...
    assign(pop, "contact", contact);
    
    assign_g(pop, "u", u, n_groups);
    assign_g(pop, "y", y, n_groups);
    assign_g(pop, "fIp", fIp, n_groups);
    assign_g(pop, "fIs", fIs, n_groups);
    assign_g(pop, "fIa", fIa, n_groups);
    assign_g(pop, "rho", rho, n_groups);
    assign_g(pop, "tau", tau, n_groups);
    
    assign(pop, "seed_times", seed_times);
    assign(pop, "dist_seed_ages", dist_seed_ages);
    assign(pop, "observer", observer);
    assign(pop, "schedule", schedule);
    ...
    seed_times = NULL, dist_seed_ages = NULL, observer = NULL, schedule = NULL)
{
    ...
    pop = list();
    for (i in 1:length(dem_locations))
    {
        pop[[i]] = cm_build_pop_SEI3R(dem_locations[i], mat_locations[i],
            dE, dIp, dIs, dIa, dH, dC, contact,
            u, y, fIp, fIa, fIs, rho, tau,
            seed_times, dist_seed_ages, observer, schedule)
    };

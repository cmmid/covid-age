...
cm_mcmc_likelihood_ = function(x, details, i)
{
    ...
    dynamics = cm_backend_simulate(parameters, details$fit_sampling, i);###details$model_seed);
    setDT(dynamics);
    dynamics = melt(dynamics, id.vars = c("run", "t", "population", "group"), variable.name = "compartment", value.name = "value");
                  ...
                  debug = F)
{
    ...
    if (debug) {
        ...
        dynamics = cm_backend_simulate(parameters, 1, details$model_seed);
        setDT(dynamics);
        ans = readline(prompt = "View results? (y/n) ");
        if (ans == "y") {
            View(dynamics);
...
cm_DIC = function(fit)
{
    ...
    dynamics = cm_backend_simulate(parameters); # TODO ADD SEED!!!...?
    setDT(dynamics);
    dynamics = melt(dynamics, id.vars = c("run", "t", "population", "group"), variable.name = "compartment", value.name = "value");

    # Calculate DIC
    D_mean = -2 * fit$likelihood_func(parameters, dynamics, fit$data, Ep);

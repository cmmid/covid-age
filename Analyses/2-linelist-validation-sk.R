...
sk_base_parameters = function(vary, fIa, report)
{
    ...
    pop$size = size;
    pop$matrices = mats;
    pop$contact = c(1, 1, 1, 1); ### TODO -- CHECK -- SCHOOLS CLOSED???
    ...
    pop$dC  = cm_delay_gamma(7.0, 4.0, t_max = 60, t_step = 0.25)$p # ?????
...
#        list(t = "2020-01-12",        contact = c(x$qH, x$qH, 0, x$qH)), # TODO CHECK!?!?
...
cm_save(fit, path(paste0("2-linelist-validation-sk-fIa-", assumed_fIa, "-", sfit, ".qs")));

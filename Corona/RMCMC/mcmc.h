...
template <typename Observables, typename LikelihoodFunc, typename ReportFunc>
void Optimize_Priors(Randomizer& R, LikelihoodFunc likelihood, ReportFunc report,
    std::vector<Distribution>& priors, unsigned int global_iter = 1000, bool local = true, bool verbose = true, 
    double tolerance = 1e-6, std::vector<double>* initial = 0, double stepsize = -1)
{
    using namespace std;
    ...
    if (global_iter > 0)
    {
        ...
        nlopt_set_maxeval(go, global_iter);
        nlopt_set_ftol_abs(go, tolerance);
    ...
    if (local)
    {
        ...
        nlopt_opt lo = nlopt_create(NLOPT_LN_NELDERMEAD, n_theta);
        ...
        nlopt_set_ftol_abs(lo, tolerance);
